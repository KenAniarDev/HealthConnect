...
(function (root, factory) {
  ...
  function (window) {
    ...
    var momDefaultOptions = {
      ...
      mapCenterLat: 9.8349, // center of the world (lat)
      mapCenterLong: 118.7384, // center of the world (long)
      mapZoomLevel: 13, // map zoom level
      mapMinZoom: 10, // map min zoom level
    ...
    var momGoogleScripts = function (apiKey, placeApi) {
      ...
      if (!window.document.getElementById("MOMScript")) {
        ...
        var dns =
          '<link id="MOMGoogleDNS" rel="dns-prefetch" href="//maps.googleapis.com">';
        ...
        var placesLibrary = "";
        if (placeApi) {
          // if place api enabled
          placesLibrary = "&libraries=places";
        }
        var script =
          '<script async defer id="MOMScript" src="https://maps.googleapis.com/maps/api/js?key=' +
          apiKey +
          placesLibrary +
          '"></script>';
        ...
        if (typeof google === "object" && typeof google.maps === "object") {
          ...
          momGoogleMap = new google.maps.Map(mapElement, {
            ...
            center: new google.maps.LatLng(
              momNewOptions.mapCenterLat,
              momNewOptions.mapCenterLong,
            ),
      ...
      if (markerObjects && markerObjects.length > 0) {
        ...
        for (var i = 0; i < markerObjects.length; i++) {
          ...
          if (typeof markersLat !== "number") {
            momConsoleError(
              'You have to set "markerObjects.markerLat" option and it must be a number. \n\n For Example: \n\n MarkersOnMap.Init({\n  markerObjects: [ \n    { \n      markerLat: 39.925018, \n    } \n  ], \n });',
            );
            //return false;
          }
          ...
          if (typeof markersLong !== "number") {
            momConsoleError(
              'You have to set "markerObjects.markerLong" option and it must be a number. \n\n For Example: \n\n MarkersOnMap.Init({\n  markerObjects: [ \n    { \n      markerLong: 32.836956, \n    } \n  ], \n });',
            );
            //return false;
          }
    ...
    var momSingleMarkerInitAndSet = function (
      mLat,
      mLong,
      mTitle,
      mSize,
      ...
      if (momNewOptions.markerOverlay) {
        ...
        markerOverlay.draw = function () {
          this.getPanes().markerLayer.classList.add(
            momNewOptions.markerOverlayClassName,
          );
        };
        ...
        var googleServiceCallback = function (results, status) {
          if (status == google.maps.places.PlacesServiceStatus.OK) {
            if (typeof results === "object" && typeof results[0] === "object") {
              ...
              var singleMedia =
                '<img class="mom-content-icon" width="100" height="100" src="' +
                singleMediaSrc +
                '" alt="MoM" />';
              ...
              if (
                place.photos &&
                (place.photos.length >= availableCount
                  ? availableCount
                ...
                for (var i = 0; i < availableCount; i++) {
                  // img style on
                  var imgStyle =
                    "max-width:100px;width:100px;height:auto;position:absolute;left:0;right:0;top:0;bottom:0;margin:auto;";
                  if (place.photos[i].width >= place.photos[i].height) {
                    var imgResizeWidth = Math.round(
                      (place.photos[i].width * 100) / place.photos[i].height,
                    );
                    var imgPosLeft = Math.round((imgResizeWidth - 100) / 2);
                    imgStyle =
                      "max-width:unset;width:" +
                      imgResizeWidth +
                      "px;height:100px;position:absolute;left:-" +
                      imgPosLeft +
                      "px;top:0;";
                  }
                  // img style off
                  singleMedia +=
                    '<div class="mom-content-photos" style="width:inherit;height:inherit;"><img class="mom-content-photo ' +
                    (place.photos[i].width >= place.photos[i].height
                      ? "ver"
                      : "hor") +
                    '" style="' +
                    imgStyle +
                    '" width="100" height="100" src="' +
                    place.photos[i].getUrl().toString() +
                    '" alt="MoM" /></div>';
                }
              }
              ...
              // total media
              contentMedia =
                '<div class="mom-content-media" style="width:100px;height:100px;position:absolute;left:0;top:0;bottom:0;margin:auto;overflow:hidden;border-radius:5px;">' +
                singleMedia +
                "</div>";
              // content media off
              ...
              if (place.name) {
                contentName =
                  '<div class="mom-content-name" style="float:left;width:100%;font-size:15px;line-height:18px;font-weight:500;margin:0 auto 8px;">' +
                  place.name +
                  "</div>";
              }
              // content name off
              ...
              if (place.formatted_address) {
                contentAddress =
                  '<div class="mom-content-address" style="float:left;width:100%;font-size:12px;line-height:14px;font-weight:400;margin:0 auto 8px;">' +
                  place.formatted_address.toString().trim() +
                  "</div>";
              }
              // content address off
              ...
              if (place.formatted_address) {
                ...
                var linkHref =
                  ...
                  ",15z";
                contentButton =
                  '<div class="mom-content-button" style="float:left;width:100%;"><a class="mom-get-directions" style="font-size:13px;line-height:13px;font-weight:bold;padding:8px 12px;display:table;margin:auto;background:#00e676;text-decoration:none;color:#fff;border-radius:20px;cursor:pointer;text-shadow:0 0 2px rgba(0, 0, 0, 0.1),0 0 10px rgba(0, 0, 0, 0.3);" href="' +
                  linkHref +
                  '" target="_blank" rel="noopener">' +
                  mButtonText.toString().trim() +
                  "</a></div>";
              }
              ...
              // total content on
              var totalContent =
                '<div style="width:300px;min-height:100px;float:left;padding-left:100px;box-sizing:border-box;position:relative;" class="mom-marker-content">' +
                contentMedia +
    ...
    var momLoadingIndicator = function (style, size, color) {
      ...
      var indicator =
        '<div id="MomLoadingIndicator" style="width:' +
        size +
        "px;height:" +
        size +
        "px;" +
        ...
        '" height="' +
        size +
        '" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid"><g transform="translate(25 50)"><circle cx="0" cy="0" r="9" fill="' +
        color +
        '" transform="scale(0.24 0.24)"><animateTransform attributeName="transform" type="scale" begin="-0.2666s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g><g transform="translate(50 50)"><circle cx="0" cy="0" r="9" fill="' +
        color +
        '" transform="scale(0.00153 0.00153)"><animateTransform attributeName="transform" type="scale" begin="-0.1333s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g><g transform="translate(75 50)"><circle cx="0" cy="0" r="9" fill="' +
        color +
        '" transform="scale(0.3 0.3)"><animateTransform attributeName="transform" type="scale" begin="0s" calcMode="spline" keySplines="0.3 0 0.7 1;0.3 0 0.7 1" values="0;1;0" keyTimes="0;0.5;1" dur="0.8s" repeatCount="indefinite"/></circle></g></svg></div>';
      return indicator;
    };
    ...
    var MarkersOnMap = {
      ...
      Init: function (momUserOptions) {
        momNewOptions = extendMarkersOnMap(
          true,
          momDefaultOptions,
          momUserOptions,
        );
        ...
        if (!apiKey) {
          // if api key undefined return false
          momConsoleError(
            'You have to set your "Google Maps Api Key" to the "googleApiKey" option. \n\n For Example: \n\n MarkersOnMap.Init({\n    googleApiKey:"YOUR_GOOGLE_MAPS_API_KEY", \n });',
          );
          return false;
        }
      ...
      Remarker: function (newMarkerObjects) {
        ...
        if (momGoogleMap) {
          ...
          // else if no Marker on the map when called "MarkersOnMap.Remarker"
            ...
            momConsoleError(
              "There is no Marker on the map. You have to initialize your map at least one Marker first.",
            );
            return false;
          }
        }
        // else Google Map not initialized yet
          ...
          momConsoleError(
            'You have to initialize your map at least one Marker first to use "MarkersOnMap.Remarker()" function.',
          );
          return false;
        }
      },
      ...
      Run: function (selector, callback) {
        ...
        if (!mapElement) {
          momConsoleError(
            'You called the MarkersOnMap with "' +
              selector +
        ...
        mapElement.innerHTML = momLoadingIndicator(
          indicatorStyles,
          60,
          "#c5c5c5",
        );
